...
class LcAmazonPayCall {
	...
	checkBeforeSubmit() {		
		...
		var deliverySelected = this.pcCheckoutForm.find('.shippingTypeSelector:checked');
	...
	loadAmazonPay(data) {
		...
		fetch(LC.global.routePaths.CHECKOUT_INTERNAL_NEXT_STEP, {
			...
			fetch(LC.global.routePaths.CHECKOUT_END_ORDER, {
                ...
                var publicKeyId = data.publicKey;
                var merchantId = data.merchantId;
                const lcAmazonPayCheckout = new LcAmazonPay();
                lcAmazonPayCheckout.loadPayButtonCheckout(merchantId, publicKeyId, payload, signature);
        	});
		});
	}
}